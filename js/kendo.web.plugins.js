...
(function ($, kendo) {
    var ExtDropDownTreeView = kendo.ui.Widget.extend({
        ...
        init: function (element, options) {
            ...
            var $element = $(element);
            $element.append(kendo.format("<input id='extDropDown{0}' class='k-ext-dropdown'/>", that._uid));
            $element.append(kendo.format("<div id='extTreeView{0}' class='k-ext-treeview s-tv' style='z-index:30;'/>", that._uid));

            // Create the dropdown.
            that._dropdown = $element.find(kendo.format("#extDropDown{0}", that._uid)).kendoDropDownList({
                dataSource: [{ text: "", value: "" }],
                dataTextField: "text",
                dataValueField: "value",
                open: function (e) {
            ...
            // Create the treeview.
            that._treeview = $element.find(kendo.format("#extTreeView{0}", that._uid)).kendoTreeView(options.treeview).data("kendoTreeView");
            ...
            // Hide the treeview.
            $treeviewRootElem
                .width(options.dropDownWidth || $dropdownRootElem.width())
                .css({
                    "border": "1px solid grey",
                    "display": "none",
                    "position": "absolute",
